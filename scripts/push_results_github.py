...
def main():
    ...
    parser.add_argument(
        "--message",
        type=str,
        default="Add calculator benchmark run results",
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
    ...
    for src in args.run_dir.glob("*.csv"):
        (run_dest / src.name).write_bytes(src.read_bytes())
    ...
    if metrics_src.exists():
        (results_root / "metrics.json").write_bytes(
